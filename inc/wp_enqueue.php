...
if ( ! function_exists('child_enqueue_styles_f3') ):
	function child_enqueue_styles_f3(){
		...
		wp_enqueue_style('swiperslider-css', $theme_url . '/assets/swiper_slider/swiper-bundle.min.css', array(), null);
		...
		wp_enqueue_script('swiperslider-js', $theme_url . '/assets/swiper_slider/swiper-bundle.min.js', array(),false, true);
		wp_enqueue_script('bootstrap-js','https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js',array(),'5.3.0',true );
		wp_enqueue_script('main-js', $theme_url . '/assets/js/main.js', array('aos-js','bootstrap-js','swiperslider-js'), filemtime($theme_path . '/assets/js/main.js'),true);


		$leadership_slider_options = function_exists('get_field') ? get_field('leadershipteam_slider_options') : null;
		wp_localize_script('main-js','globalData', array(
			'nonce' => wp_create_nonce('wp_rest'),
			'siteURL' => home_url(),
			'testStr' => 'Test string... (`wp_enqueue.php` `wp_localize_script` function)', 

            /*ACF- SwiperJS Slider in "About Us" Page ("LeaderShip Team")*/
			'leadershipTeamSlider' => array(
                'autoplay' => !empty($leadership_slider_options['leadershipteam_slider_options_slider_autoplay']),
                'navigation' => !empty($leadership_slider_options['leadershipteam_slider_options_slider_navigation']),
                'speed' => !empty($leadership_slider_options['leadershipteam_slider_options_speed']) ? intval($leadership_slider_options['leadershipteam_slider_options_speed']) * 1000 : 2000,
            ),
            /*__/ACF- SwiperJS Slider in "About Us" Page ("LeaderShip Team")*/
		)); 
	} 
